...
if sys.platform == "win32":
    ...
    # -------------------------------------------------------------------------
    # MENTOR FIX: Host Compiler (GCC) Anpassungen für PyTorch 2.5 + CUDA 12.6
    # -------------------------------------------------------------------------
    CC_FLAGS += ["-fopenmp", "-std=c++17", "-DNVTX_DISABLE"]

if "darwin" in platform:
    CC_FLAGS += ["-stdlib=libc++", "-std=c++17"]

# -------------------------------------------------------------------------
# MENTOR FIX: NVCC Compiler Anpassungen für Orin + CUDA 12.6
# -------------------------------------------------------------------------
NVCC_FLAGS += [
    "--expt-relaxed-constexpr", 
    "--expt-extended-lambda",
    "-std=c++17",             # Zwingend für PyTorch 2.5
    "-DNVTX_DISABLE",         # Fix für "optional deduction failed"
    "-DCUB_SKIP_NVTX_CHECK"   # Fix für CUB Konflikte
]
...
if not (BLAS is False):
    ...
    for blas in BLAS_LIST:
        ...
        raise ImportError('BLAS not found.')
...
SOURCE_SETS = {
    "cpu": [
        CppExtension,
        [
            "math_functions_cpu.cpp", "coordinate_map_manager.cpp", "convolution_cpu.cpp",
            "convolution_transpose_cpu.cpp", "local_pooling_cpu.cpp", "local_pooling_transpose_cpu.cpp",
            "global_pooling_cpu.cpp", "broadcast_cpu.cpp", "pruning_cpu.cpp",
            "interpolation_cpu.cpp", "quantization.cpp", "direct_max_pool.cpp",
        ],
    ...
    "gpu": [
        CUDAExtension,
        [
            "math_functions_cpu.cpp", "math_functions_gpu.cu", "coordinate_map_manager.cu",
            "coordinate_map_gpu.cu", "convolution_kernel.cu", "convolution_gpu.cu",
            "convolution_transpose_gpu.cu", "pooling_avg_kernel.cu", "pooling_max_kernel.cu",
            "local_pooling_gpu.cu", "local_pooling_transpose_gpu.cu", "global_pooling_gpu.cu",
            "broadcast_kernel.cu", "broadcast_gpu.cu", "pruning_gpu.cu",
            "interpolation_gpu.cu", "spmm.cu", "gpu.cu", "quantization.cpp", "direct_max_pool.cpp",
        ],
        ["pybind/minkowski.cu"],
        [],
    ],
...
setup(
    ...
    keywords=["pytorch", "Minkowski Engine", "Sparse Tensor", "3D Vision"],
    zip_safe=False,
    python_requires=">=3.6",
)
